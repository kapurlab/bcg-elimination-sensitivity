...
res <- bind_rows(lapply(seq_len(nrow(design)), function(i) {
  ...
  et <- stoch_revacc(d$R0, d$N, BASE$e_s, BASE$e_i, D = sc$D, k = 20, p = 1,
  ...
  data.frame(d, p_free_10y = mean(et <= 10), p_free_20y = mean(et <= 20), p_free_50y = mean(et <= 50),
             ...
             Rv = R_vacc(d$R0, BASE$e_s, BASE$e_i, sc$D, 1))
}))
write.csv(res, "output/tables/stochastic_grid.csv", row.names = FALSE)

# no-vaccination reference: chance fade-out alone, same grid
ref <- bind_rows(lapply(N_grid, function(n) bind_rows(lapply(R0_grid, function(r) {
  et <- stoch_revacc(r, n, 0, 0, D = Inf, k = 1, p = 0, reps = REPS, years = YEARS, revacc = FALSE)
  data.frame(R0 = r, N = n, p_free_20y = mean(et <= 20), p_free_50y = mean(et <= 50))
}))))
write.csv(ref, "output/tables/stochastic_grid_novacc.csv", row.names = FALSE)
...
g22 <- ggplot(d, aes(R0f, Nf, fill = p_free_20y)) +
  ...
  labs(x = "Within-herd R0", y = "Herd size (animals)",
       title = "Stochastic elimination on a conceptual grid: herd size against R0, four programmes",
...
ggsave("output/figs/fig22_stochastic_grid_20y.png", g22, width = 12, height = 9, dpi = 200, bg = "white")
...
g23 <- ggplot(d2, aes(R0f, Nf, fill = fill_val)) +
  ...
  labs(x = "Within-herd R0", y = "Herd size (animals)",
       ...
       subtitle = "Small herds fade out by chance even when the reproduction number under vaccination is above 1; large herds need it below 1.") +
  theme_pub(11) + theme(plot.title.position = "plot", panel.grid = element_blank(), panel.border = element_blank(), legend.key.width = unit(1.6, "cm"))
ggsave("output/figs/fig23_stochastic_grid_median.png", g23, width = 12, height = 9, dpi = 200, bg = "white")
